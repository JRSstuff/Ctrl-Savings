...
This document chronicles all the real-world engineering challenges, UI/UX refinements, database integration hurdles, security vulnerabilities, and deployment obstacles encountered throughout the day, detailing the exact root causes and implemented solutions.
...
At the conclusion of Day 1, all foundational architectural layers of the **Ctrl+Savings** application are operational, robust, and verified:
1. **Frontend**: Zero-lag Svelte 5 single-page application with custom high-contrast styling and shared-element transitions.
2. **PWA**: Fully installable on iOS, Android, and Desktop with automated Service Worker precaching and offline operation.
3. **Backend**: Dedicated serverless API concealing database endpoints from end-users.
4. **Database**: Secure, authenticated PostgreSQL table with salted cryptographic hashing and dual-format legacy compatibility.
...
* **Requirement**: Expand the registration flow to include firstName, middleName, lastName, and dateOfBirth.
* **Solution**:
  - Updated database_schema.md with an ALTER TABLE script for Supabase.
  - Modified api/register.js and api/login.js to accept and return these new fields.
  - Added new inputs to AuthView.svelte and cached the profile details in localStorage.
  ...
  - Bound ProfileView.svelte to read the profile to display dynamically.
...
* **Problem**: In the profile view, the user avatar showed 'U' with name 'User' and 'DOB: Not Set', even though the pp_users table had valid data (	est sd, 1999-11-22).
* **Root Cause**: In ite.config.js (local preview/development server), the mock /api/login and /api/register endpoints only queried 'id, username, password' and returned { data: data.id } (only the string ID). Consequently, AuthView.svelte stored undefined in localStorage for llowance_firstname, llowance_lastname, llowance_middlename, and llowance_dob.
* **Solution**: 
  - Updated ite.config.js to select all user columns (id, username, first_name, middle_name, last_name, date_of_birth, total_budget, available_budget) and return the entire user object with passwords safely stripped.
  - Created a dedicated /api/user.js endpoint (and mirrored it in ite.config.js) so the client can query the database directly by user ID for fresh, real-time profile data.
